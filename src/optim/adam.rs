...
impl PhoenixOptimizer for Adam {
    ...
    fn load_state_dict(&mut self, state_dict: &HashMap<String, crate::nn::optimizer::OptimizerValue>) -> Result<(), OptimizerError> {
        use crate::nn::optimizer::OptimizerValue;
            ...
            if let Some(state_key) = key.strip_prefix("state.") {
                if let Some(dot_pos) = state_key.find('.') {
                    ...
                    match (field, value) {
                        ...
                        _ => return Err(OptimizerError::InvalidStateDict(format!("Unknown state field: {}", field))),
                    }
                }
            }
        }
