...
impl Metric for Accuracy {
    fn update(&mut self, preds: &Tensor, target: &Tensor) {
        ...
        if self.top_k == 1 {
            ...
            let correct = top_indices
                .eq_tensor(&target_expanded)
...
impl MeanSquaredError {
    pub fn new() -> Self {
        Self { sum_squared_error: 0.0, count: 0 }
    }
}
...
impl MeanAbsoluteError {
    pub fn new() -> Self {
        Self { sum_absolute_error: 0.0, count: 0 }
    }
}
...
impl F1Score {
    pub fn new(threshold: f64) -> Self {
        Self { true_positives: 0, false_positives: 0, false_negatives: 0, threshold }
    }
}
...
impl MetricCollection {
    ...
    pub fn compute(&self) -> HashMap<String, f64> {
        self.metrics.iter().map(|(name, metric)| (name.clone(), metric.compute())).collect()
    }
...
impl AUROC {
    ...
    fn compute_auroc(&self) -> f64 {
        ...
        // Sort by predictions
        let mut paired: Vec<(f64, i64)> =
            self.predictions.iter().zip(self.targets.iter()).map(|(&p, &t)| (p, t)).collect();
