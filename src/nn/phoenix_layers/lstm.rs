//! LSTM layer with PhoenixModule trait implementation
...
impl LSTM {
    ...
    pub fn builder(input_size: i64, hidden_size: i64) -> LSTMBuilder {
        LSTMBuilder {
            input_size,
            hidden_size,
            num_layers: 1,
        ...
        for layer in 0..self.num_layers {
            for direction in 0..num_directions {
                ...
                let weight_ih = Tensor::randn(
                    &[4 * self.hidden_size, layer_input_size],
                    (Kind::Float, device),
                ) / (layer_input_size as f64).sqrt();
                self.weight_ih.push(weight_ih);

                // Initialize weights for hidden-hidden
                ...
                if self.bias {
                    ...
                    let bias_ih = Tensor::zeros(&[4 * self.hidden_size], (Kind::Float, device));
                    let bias_hh = Tensor::zeros(&[4 * self.hidden_size], (Kind::Float, device));
    ...
    pub fn forward_with_state(
        ...
        let (h_0, c_0) = state.unwrap_or_else(|| {
            let h = Tensor::zeros(
        ...
        // Use libtorch's LSTM implementation
        let (output, h_n, c_n) = if self.bias {
            input.lstm(
                &[&h_0, &c_0],
...
impl PhoenixModule for LSTM {
    ...
    fn load_state_dict(&mut self, state_dict: &HashMap<String, Tensor>) -> Result<(), PhoenixModuleError> {
        // Load weights and biases from state dict
        for (key, tensor) in state_dict {
            if key.starts_with("weight_ih") {
                // Parse layer index from key
                // Implementation would extract layer index and update corresponding weight
            } else if key.starts_with("weight_hh") {
                // Similar parsing for hidden weights
            }
            // Similar for biases
        }
        Ok(())
