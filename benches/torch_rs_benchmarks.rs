...
fn bench_conv2d_forward(b: &mut Bencher) {
    let layer = Conv2d::builder(3, 64, 3)
        .stride(1)
        .padding(1)
        .build();
...
fn bench_transformer_encoder(b: &mut Bencher) {
    let encoder = TransformerEncoderLayer::builder(512, 8)
...
fn bench_sgd_step(b: &mut Bencher) {
    ...
    let param_ptrs: Vec<_> = params.iter().map(|p| p as *const Tensor as *mut Tensor).collect();
    let mut optimizer = SGD::builder(param_ptrs.into_iter())
        .lr(0.1)
        .momentum(0.9)
        .build()
        .unwrap();
...
mod gpu_benchmarks {
    ...
    fn bench_gpu_transformer(b: &mut Bencher) {
        let device = Device::Cuda(0);
        let mut encoder = TransformerEncoderLayer::builder(512, 8)
            .dim_feedforward(2048)
            .build();
